...
public class HTTPResponseStatusInfo {
    ...
    private String hint;
    public static HTTPResponseStatusInfo getHTTPResponseStatus(Context c, int code)
    {
        return getHTTPResponseStatus(c,String.valueOf(code));
    }
    public static HTTPResponseStatusInfo getHTTPResponseStatus(Context c, String code)
    {
        if(StringUtil.isEmpty(code))
            return null;
        HTTPResponseStatusInfo hrs = new HTTPResponseStatusInfo();
        hrs.code = code;
        hrs.desc = ResourceUtil.getString(c,ResourceUtil.$id(c,CSTR_HRS_DESC+code, ResourceUtil.TYPE.STRING));
        hrs.desccn = ResourceUtil.getString(c,ResourceUtil.$id(c,CSTR_HRS_DESCCN+code, ResourceUtil.TYPE.STRING));
        hrs.hint = ResourceUtil.getString(c,ResourceUtil.$id(c,CSTR_HRS_HINT+code, ResourceUtil.TYPE.STRING));
    ...
    服务器由于遇到错误而不能完成该请求。
            ...
            　· 500.18 – java.net.URL 授权存储不能打开。这个错误代码为 IIS 6.0 所专用。
            　· 500.100 - 内部 ASP 错误。
